...
try:
    with open('best_elastic_net_model.pkl', 'rb') as file:
        best_elastic_net_model = pickle.load(file)
    with open ('scaler.pkl', 'rb') as file:
        scaler = pickle.load(file)
except FileNotFoundError:
    st.error("Error: 'scaler.pkl' or 'best_elastic_net_model' not found.")
    st.stop()
except pickle.PickleError:
    st.error("Error: Failed to load 'scaler.pkl' or 'best_elastic_net_model'. The files might be corrupted.")
    st.stop()
...
def run_ml_app():
    ...
    instrumentalness = right.number_input('Instrumentalness', min_value=0.00, max_value=1.00, step=0.01)
    ...
    data = pd.DataFrame(features, columns=[
        'song_duration_ms', 'acousticness', 'danceability', 'instrumentalness', 'liveness', 'loudness', 'audio_mode',
        'speechiness', 'tempo', 'audio_valence', 'time_signature_1', 'time_signature_4', 'time_signature_5', 'key_1',
